...
*(Personal lab notebook - Last verified 2025‑05-10)*
...
> 5. **ggml → Metal / CPU** — The dispatcher off-loads GEMM/conv kernels to Metal's 80-core GPU shaders, leaving residual ops to Accelerate-BLAS on the CPU.
...
* **Why it matters:** every higher-level component—`llama.cpp`, Whisper, RWKV—builds its computation graph on ggml and lets ggml decide *which* kernels (CPU, Metal, BLAS) to run.
* **Metal integration:** when `llama.cpp` is built with `-DLLAMA_METAL=ON`, ggml's dispatcher swaps heavy ops into Metal shaders automatically.
...
| --------------------------------- | ----------------------------------------------------------------------------- |
| **Fast local chat** without code  | Open WebUI → Ollama → llama.cpp → ggml/Metal                                  |
| **Python notebook** prototyping   | `import llama_cpp` → llama-cpp-python → llama.cpp → ggml/Metal                |
| **Go micro-service**              | Directly embed Ollama's Go client or cgo bindings                             |
| **Kernel hacking / perf tracing** | Run `llama` CLI built from source (`-DLLAMA_METAL=ON`); bypass Ollama & WebUI |
...
4. **ggml** dispatches GPU-able nodes to **Metal** kernels, leaves the rest to CPU SIMD/BLAS.
...
| ---------------- | -------------------------------- | -------------------------------------------------- |
| ggml             | none                             | CPU kernels, optional Metal shaders if compiled    |
| llama.cpp        | cmake, clang/Xcode               | vendored ggml copy                                 |
| llama-cpp-python | `pip`, Python ≥3.9               | wheel embeds matching `libllama`                   |
| Ollama           | Go ≥1.21                         | vendor copy of llama.cpp & ggml compiled for Metal |
...
* **`-DLLAMA_METAL=ON`** → compiles `ggml-metal.metallib`, off-loads layers to the on-die GPU.
...
* `llama_cpp.llama_print_system_info()` **exists** — use it to verify Metal/Accelerate flags.
...
| ------------------------------ | ------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------- | ------------------------------ |
| **Metal : EMBED\_LIBRARY = 1** | ggml-metal was compiled in and the shader library is baked directly into `libllama.dylib`. | One-box GPU off-load will work without needing an external `.metallib`. |                                |
...
* **GPU off-load confirmed:** backend shows "Metal" and your pp512 rate is three-digit.
